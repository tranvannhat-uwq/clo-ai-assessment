...
import { updateStudentProfile, changeStudentPassword } from '@/actions/student/profile'
import styles from '../../lecturer/page.module.css'

export function StudentProfileClient({ profile, email }: { profile: any; email: string }) {
  ...
  return (
    <div>
      ...
      <p style={{ marginBottom: '2rem', color: '#64748b', fontSize: '1.05rem' }}>
        Quản lý thông tin cá nhân và bảo mật tài khoản sinh viên.
      </p>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', maxWidth: '900px' }}>
        {/* Cập nhật thông tin */}
        <div className={styles.card} style={{ padding: '1.5rem' }}>
          <h3 style={{ margin: '0 0 1.25rem', fontSize: '1.1rem', color: '#1e3a8a', borderBottom: '2px solid #e2e8f0', paddingBottom: '0.5rem' }}>
            Thông tin sinh viên
          </h3>
          ...
          <form onSubmit={handleUpdateInfo} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
               <div>
                 <label style={labelStyle}>Mã SV</label>
                 <input type="text" value={profile?.code || ''} disabled style={{ ...inputStyle, background: '#f8fafc', color: '#64748b' }} />
               </div>
               <div>
                 <label style={labelStyle}>Lớp hành chính</label>
                 <input type="text" value={profile?.student_classes?.name || 'Chưa gán'} disabled style={{ ...inputStyle, background: '#f8fafc', color: '#64748b' }} />
               </div>
            </div>
            <div>
              <label style={labelStyle}>Email (Tài khoản)</label>
              <input type="text" value={email} disabled style={{ ...inputStyle, background: '#f8fafc', color: '#64748b' }} />
            </div>
            <div>
            ...
            <button
              ...
              style={{ padding: '0.7rem', background: '#1e3a8a', color: 'white', border: 'none', borderRadius: '0.375rem', fontWeight: 600, cursor: 'pointer', opacity: infoLoading ? 0.6 : 1, fontSize: '0.95rem' }}
            >
        ...
        <div className={styles.card} style={{ padding: '1.5rem', alignSelf: 'start' }}>
          ...
          <form onSubmit={handleChangePassword} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <button
              type="submit"
              disabled={pwLoading}
              style={{ padding: '0.7rem', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '0.375rem', fontWeight: 600, cursor: 'pointer', opacity: pwLoading ? 0.6 : 1, fontSize: '0.95rem' }}
            >
